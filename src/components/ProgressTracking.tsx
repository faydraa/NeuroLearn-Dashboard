...
export function ProgressTracking({ userName }: ProgressTrackingProps) {
  ...
  const monthStats = useMemo(() => {
    ...
    const avgFocus = focusDays.length > 0
        ? Math.round(focusDays.reduce((sum, d) => sum + d.avgFocus, 0) / focusDays.length)
        : 0;
    let streak = 0;
    for (let i = currentMonthDaysData.length - 1; i >= 0; i--) {
      if (currentMonthDaysData[i].duration > 0) streak++;
  ...
  const focusBreakdownInsights = useMemo(() => {
    ...
    return [
      ...
      { 
        title: focused >= 70 ? "High focus time" : focused >= 55 ? "Moderate focus time" : "Low focus time",
        text: focused >= 70
    ...
    <div className="max-w-7xl mx-auto p-8">
      ...
      <div className="flex gap-2 mb-6">
        <button
          ...
          className={`px-6 py-2 rounded-lg font-medium transition ${viewMode === "day" ? "bg-indigo-600 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`}
        >
          Daily Report
        </button>
        <button
          onClick={() => setViewMode("month")}
          className={`px-6 py-2 rounded-lg font-medium transition ${viewMode === "month" ? "bg-indigo-600 text-white" : "bg-gray-100 text-gray-700 hover:bg-gray-200"}`}
        >
          Monthly
        </button>
      </div>
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <div className="h-3 bg-gray-200 rounded-full overflow-hidden">
                    <div className="h-full bg-green-500" style={{ width: `${dailyAnalysis?.focusedPercent ?? 0}%` }} />
                  </div>
                </div>
                <div>
                  <div className="flex items-center justify-between text-sm mb-1">
                    <span className="text-gray-700">Unfocused Time</span>
                    <span className="font-semibold text-gray-900">{100 - (dailyAnalysis?.focusedPercent ?? 0)}%</span>
                  </div>
                  <div className="h-3 bg-gray-200 rounded-full overflow-hidden">
                    <div className="h-full bg-amber-500" style={{ width: `${100 - (dailyAnalysis?.focusedPercent ?? 0)}%` }} />
                  </div>
                </div>
      ...
      {viewMode === "month" && (
        <>
          <div className="mb-4">
            <p className="text-sm text-gray-600">Showing monthly summary for <span className="font-semibold text-gray-900">{currentMonthLabel}</span></p>
          </div>
          <div className="grid md:grid-cols-4 gap-6 mb-8">
            <div className="bg-gradient-to-br from-blue-500 to-blue-600 rounded-xl p-6 text-white shadow-sm">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              ...
              <ResponsiveContainer width="100%" height={260}>
                <BarChart data={monthlyChartData}>
                  <CartesianGrid strokeDasharray="3 3" stroke="#f0f0f0" />
                  <XAxis dataKey="day" type="number" domain={[1, currentMonthDaysData.length]} allowDecimals={false} tick={{ fontSize: 11 }} />
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <div className="flex items-center gap-3 text-sm text-gray-600">
                <div className="flex items-center gap-1"><div className="w-3 h-3 bg-gray-200 rounded" /><span>No study</span></div>
                <div className="flex items-center gap-1"><div className="w-3 h-3 bg-green-200 rounded" /><span>Light</span></div>
                <div className="flex items-center gap-1"><div className="w-3 h-3 bg-green-600 rounded" /><span>Heavy</span></div>
              </div>
            </div>
            <div className="flex items-center justify-between mb-4">
              <button onClick={goPrevMonth} className="px-3 py-2 rounded-lg bg-gray-100 hover:bg-gray-200 transition"><ChevronLeft className="w-4 h-4" /></button>
              <p className="font-semibold text-gray-900">{calendarLabel}</p>
              <button onClick={goNextMonth} className="px-3 py-2 rounded-lg bg-gray-100 hover:bg-gray-200 transition"><ChevronRight className="w-4 h-4" /></button>
            </div>
            <div className="grid grid-cols-7 gap-2">
              {Array.from({ length: calendarStartsOn }).map((_, i) => <div key={`blank-${i}`} className="h-14" />)}
              {calendarCells.map((cell) => (
                <div key={cell.iso} className={`h-14 rounded-lg p-2 ${intensityClass(cell.duration)} ${cell.iso === todayStr ? "ring-2 ring-indigo-500" : ""}`}>
